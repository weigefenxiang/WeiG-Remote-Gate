...
Gate selected-plan OPEN authority now has explicit software coverage on `dev`. OPEN requires the current authorized source plus the selected family `device + ingress_port + scope` profile; a same-profile runtime for a different source and any active runtime outside the exact selected AccessPlan are shown as `OPEN ELSEWHERE` and remain Close-only. A Dual selection with only one family still reported active is shown as `PARTIAL OPEN` and also remains Close-only. A failed dashboard refresh immediately removes cached OPEN display authority without introducing a second browser freshness clock; last-known runtime evidence may keep only the safe Close path. The Server queue independently enforces the same close-before-switch boundary by rejecting replacement Activate while the last reported firewall still says any Gate family is active; Internet Exit state is intentionally not part of that Gate-runtime guard. `tests/test_gate_profile_binding_contract.py` and `tests/test_gate_active_runtime_guard.py` provide routine executable/static evidence, while `tests/browser_gate_profile_binding.mjs` remains a `main`-only/manual release Browser Matrix regression. Therefore this is software evidence only: Browser Matrix PASS and hardware PASS do not change.

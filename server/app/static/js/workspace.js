(() => {
  const KEY = 'remote-gate:workspace:v1';
  ...
  if (Array.isArray(saved.order)) {
    ...
    saved.order.forEach((id) => { if (map.has(id)) workspace.append(map.get(id)); });
  }
  cards().forEach((card) => {
  ...
  resetButton?.addEventListener('click', () => {
    ...
    const map = new Map(cards().map((card) => [card.dataset.cardId, card]));
    order.forEach((id) => { if (map.has(id)) workspace.append(map.get(id)); });

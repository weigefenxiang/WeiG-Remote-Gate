...
try {
  for (const [width, height] of viewports) {
    ...
    assert(body.egress_wan === 'WAN2', `${width}x${height}: automatic IPv4 exit did not choose WAN2`);
    ...
    const manualIpv6 = await page.evaluate(() => ({
      ...
      exitMode: document.querySelector('#egress-mode-segment .active')?.dataset.egressMode,
      exitWan: document.querySelector('#egress-ipv6-select')?.value,
    }));
    ...
    assert(manualIpv6.exitMode === 'ipv6' && manualIpv6.exitWan === 'WAN2', `${width}x${height}: Access WAN change incorrectly rewrote independent IPv6 Internet Exit`);
  ...
  // Dual-stack regression: same-WAN public Access is selected automatically; Exit stays a mode plus two scalar WANs.
  ...
  await dualPage.locator('#egress-ipv4-select-picker-trigger').click();
  await dualPage.waitForSelector('#endpoint-picker-layer.open .endpoint-option-card.selected');
  const exit4Blocks = await dualPage.locator('#endpoint-picker-layer .endpoint-option-card.selected .path-family-block').count();
  assert(exit4Blocks === 1, `Dual Internet Exit IPv4 selector must render one FamilyPathBlock (${exit4Blocks})`);
  await dualPage.keyboard.press('Escape');
  await dualPage.locator('#egress-ipv6-select-picker-trigger').click();
  await dualPage.waitForSelector('#endpoint-picker-layer.open .endpoint-option-card.selected');
  const exit6Blocks = await dualPage.locator('#endpoint-picker-layer .endpoint-option-card.selected .path-family-block').count();
  assert(exit6Blocks === 1, `Dual Internet Exit IPv6 selector must render one FamilyPathBlock (${exit6Blocks})`);
  await dualPage.keyboard.press('Escape');
  ...
  assert(dualBody.egress_wan === 'WAN2', 'same-WAN dual exit did not choose WAN2');
  ...
  console.log(`Browser layout regression passed for ${viewports.length} viewports plus mode-first Internet Exit, independent family WAN selectors, shared-WAN Dual default, Access PathCard rendering, transaction lock, candidate recovery, and observed-source probe suppression.`);
} finally {
  await browser.close();
}

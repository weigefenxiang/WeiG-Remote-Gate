...
class UIContractTests(unittest.TestCase):
    ...
    def test_gate_transaction_lock_is_real_and_server_terminal_owned(self):
        ...
        self.assertIn("closeCanPreempt", gate)
        self.assertIn("form.inert = Boolean(locked && !preemptible)", gate)
        self.assertIn("const preemptingClose = action === 'close' && closeCanPreempt();", gate)
        self.assertIn("if(closeCanPreempt()&&target.id==='close-button')return;", gate)

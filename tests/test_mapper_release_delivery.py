...
class MapperReleaseDeliveryTests(unittest.TestCase):
    ...
    def test_install_and_update_use_release_helper_without_uname_fallback(self):
        ...
        self.assertIn('mapper_channel=release', UPDATE_SH)
        self.assertIn('[ "$RAW_REF" = dev ] && mapper_channel=dev', UPDATE_SH)
        self.assertIn('"install-$mapper_channel"', UPDATE_SH)
        helper = INSTALLER.read_text(encoding="utf-8")
        self.assertIn('install-release', helper)
        self.assertIn('install-dev', helper)
